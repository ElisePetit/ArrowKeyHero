...
public class CubeMoveM : MonoBehaviour {
	...
	void FixedUpdate () {
        ...
        transform.Translate(new Vector3(0,0,-3f) * (speed * UnityEngine.Random.value + 0.1f) * Time.fixedDeltaTime);
        CheckKeys(rend);

        if (transform.position.z < -3)
        {
            Debug.Log("Boom ! Raté au milieu");
